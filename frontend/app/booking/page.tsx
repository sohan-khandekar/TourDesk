...
function Stars({ r }: { r: number }) {
  return (
    <span className="flex items-center gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Star key={i} size={12}
          className={cn('fill-current', i <= Math.round(r) ? 'text-amber-400' : 'text-slate-200')} />
      ))}
...
function BookingContent() {
  ...
  if (!place) return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="flex flex-col items-center justify-center py-48 text-slate-400">
        ...
        <p className="text-2xl font-black text-slate-600 mb-2">Place not found</p>
        <Link href="/" className="mt-2 text-orange-500 text-sm font-semibold hover:underline">
          &larr; Back to home
        </Link>
      </div>
    </div>
  ...
  const childPrice = place.ticketPriceChild ?? Math.round(place.ticketPrice * 0.6);
      ...
      <div className="pt-16">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <Link href={`/places?city=${city}`}
            ...
            <ArrowLeft size={15} /> Back to {city.charAt(0).toUpperCase()+city.slice(1)}
          </Link>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-4 shadow-sm">
                <div>
                  ...
                  <h1 className="text-xl font-black text-navy-900 mt-2 leading-tight">{place.name}</h1>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { label: 'Best Time',   val: place.bestTimeToVisit,  icon: Calendar },
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-100 flex items-center gap-3">
                  <div className={cn(
                    ...
                    {isFood ? <UtensilsCrossed size={20} className="text-rose-600" /> : <Ticket size={20} className="text-orange-600" />}
                  </div>
                  <div>
                    <h2 className="font-black text-navy-900 text-lg">
                      {isFood ? 'Popular dishes to try' : 'Official ticket redirect'}
                    </h2>
                    <p className="text-slate-400 text-xs">
                      {isFood ? 'No booking on TourDesk — just food recommendations' : 'TourDesk shows details, then sends you to the official portal'}
                    </p>
                  </div>
                </div>
                  ...
                  {isFood ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {(place.dishesToTry ?? place.highlights).map(dish => (
                          <div key={dish} className="rounded-xl border border-rose-100 bg-rose-50 p-4">
                      ...
                      <div className="flex items-start gap-2 text-xs text-slate-500 bg-slate-50 rounded-xl p-3">
                        ...
                        Restaurants and street-food places are shown as discovery guides only. TourDesk does not reserve tables, collect money, or place food orders.
                      </div>

                      {place.officialInfoLink && (
                        <a href={place.officialInfoLink} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="space-y-5">
                      <div className="rounded-2xl border border-orange-100 bg-orange-50 p-5">
                        <p className="text-xs font-black text-orange-500 uppercase tracking-wider mb-3">Price structure</p>
                        <p className="text-sm text-slate-700 leading-relaxed mb-4">{place.priceDetails}</p>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div className="bg-white rounded-xl border border-orange-100 p-4">
                            <p className="text-xs text-slate-400 font-bold mb-1">Adult starts from</p>
                            {place.ticketPrice === 0 ? (
                          ...
                          <div className="bg-white rounded-xl border border-orange-100 p-4">
                            <p className="text-xs text-slate-400 font-bold mb-1">Child starts from</p>
                            {place.ticketPrice === 0 ? (
                              <p className="text-xl font-black text-emerald-600">Free</p>
                            ) : (
                              <p className="text-xl font-black text-orange-600 flex items-center gap-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                          ...
                          <p className="text-sm font-semibold text-slate-700">{place.location}</p>
                        </div>
                        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                          <p className="text-xs text-slate-400 font-bold uppercase mb-1">Timings</p>
                          <p className="text-sm font-semibold text-slate-700">{place.timings}</p>
                        </div>
                      </div>
