...
function Stars({ rating }: { rating: number }) {
  return (
    <span className="flex items-center gap-0.5">
      ...
      <span className="ml-1 text-slate-500 text-xs font-semibold">
        {rating}
      </span>
    </span>
  );
...
export default function PlaceCard({
  ...
  const cat = CAT_STYLE[place.category] ?? {
    bg: "bg-slate-100",
    text: "text-slate-700",
  };
  ...
  return (
    ...
    >
      <div className="relative h-48 bg-gradient-to-br from-slate-100 to-slate-200 overflow-hidden flex-shrink-0">
        ...
        <span
          className={cn(
            "absolute top-3 left-3 text-xs font-bold px-2.5 py-1 rounded-full capitalize",
            cat.bg,
            cat.text,
          )}
        >
        ...
        >
          <Heart
            ...
            className={cn(
              "transition-colors",
              isVisited ? "fill-orange-500 text-orange-500" : "text-slate-400",
            )}
          />
        </button>
      ...
      {isVisited && (
        ...
        >
          <CheckCircle2
            size={12}
            className="absolute -top-[30px] right-0.5 text-white"
          />
        </div>
      )}
    </div>
